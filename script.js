...
// Tüm veriler kendi sitede gösterilir
...
class WeatherApp {
    ...
    async fetchWeatherByCoords(lat, lon, cityName) {
        try {
            ...
            this.showError(error.message || 'Bir hata oluştu. Lütfen tekrar deneyin.');
        }
    }
    ...
    async loadDefaultCity() {
        ...
        try {
            ...
            if (!geocodeResponse.ok) {
                throw new Error('Varsayılan şehir yüklenemedi');
            }
            ...
            if (geocodeData.results && geocodeData.results.length > 0) {
                ...
                this.hideLoading();
                this.showError('Varsayılan şehir bulunamadı. Lütfen bir şehir arayın.');
            }
        } catch (error) {
            console.error('Varsayılan şehir yüklenemedi:', error);
            this.hideLoading();
            this.showError('Başlangıç verisi yüklenemedi. Lütfen bir şehir arayın.');
        }
    }
